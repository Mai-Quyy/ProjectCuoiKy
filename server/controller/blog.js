...
1. Check xem người đó có dislike hay không => nếu có thì bỏ DISLIKE
2. check xem người đó trước đó có like hay không => bỏ like / thêm like
 */
// pull keo id trong mang ra ngoai
// push theem vao`
...
// const excludedFields = "-refreshToken -password -role -created -updateAt";
...
const deleteBlog = asyncHandler(async (req, res) => {
  ...
  return res.json({
    success: blog ? true : false,
    delete: blog || "something wrong",
  });
});
uploadImageBlog = asyncHandler(async (req, res) => {
  const { bid } = req.params;
  if (!req.file) throw new Error("Mssing input");
  const response = await Blog.findByIdAndUpdate(
    bid,
    { image: req.file.path },
    { new: true }
  ...
  return res.status(200).json({
    ...
    updatedBlog: response ? response : "Cannot upload images product",
  });
});
module.exports = {
  createNewBlog,
